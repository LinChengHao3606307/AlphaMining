...
class AttentionNet(nn.Module):
    def __init__(self,ip1_size:int,ip2_size:int = None,out_size:int = None,mid_size = None):
        super().__init__()
        self.ip1_size = ip1_size
        self.ip2_size = ip2_size
        self.out_size = out_size
        self.mid_size = mid_size
        if ip2_size is None:
            self.ip2_size = ip1_size
        if out_size is None:
            self.out_size = ip1_size
        if mid_size is None:
            self.mid_size = ip1_size*2
        self.mlp1 = nn.Sequential(
            nn.Linear(self.ip1_size, self.mid_size),
            nn.Dropout(0.1),
            nn.ReLU(),
    ...
    def forward(self,ip1,ip2):
        return F.softmax(
            (self.mlp1(ip1)*ip2).sum(dim=-1)
            ,dim=-1)

class LSTM_ag(nn.Module):
    def __init__(
            self,
            ...
            save_path:str = os.path.join(DefaultValues.model_state_dict_path, "LSTM_ag")
        ...
        self.ip1_idx_output = AttentionNet(lstm_hidden_size)
        self.ip2_idx_output = AttentionNet(lstm_hidden_size)
    ...
    def initialize_weights(self):
        ...
        # Initialize AttentionNet weights
        for mlp in [self.ip1_idx_output.mlp1,
                    self.ip2_idx_output.mlp1]:
            for layer in mlp:
                if isinstance(layer, nn.Linear):
    ...
    def _get(self, all_formulas, idx, t_shift, t_span):
        if idx.detach().cpu().item() < 0:
            ...
            self.hidden_states_buffer[idx]=re[0][-1]
            return re
        ...
        if type_idx.item() == DefaultValues.getter_fn_idx:
            re = self._get(all_formulas, id1, t_shift1, t_span1)
            self.hidden_states_buffer[idx]=re[0][-1]
            return re
        ...
        current_ip = self.fn_profile_fuser(current_ip).unsqueeze(0)

        slm1 = self._get(all_formulas, id1, t_shift1+t_shift, t_span1+t_span)
        slm2 = self._get(all_formulas, id2, t_shift2+t_shift, t_span2+t_span)

        new_slm = (
            (slm1[0]+slm2[0])/2,# + self.sm_fuser(torch.cat([slm1[0],slm2[0]],dim=-1)),
            (slm1[1]+slm2[1])/2# + self.lm_fuser(torch.cat([slm1[1],slm2[1]],dim=-1))
        )
        _, re = self.lstm_cell(current_ip, new_slm)
    ...
    def forward(self, input_state: torch.Tensor):
        ...
        for i in range(b):
            ...
            all_hidden = self.hidden_states_buffer[:hidden_s_valid_dim]
            all_pre_hidden = all_hidden[:-1]
            last_hidden = all_hidden[-1].unsqueeze(0)
            # action output
            fn_types_probs = F.softmax(self.fn_types_output(last_hidden), dim=-1)

            prob_valid_dim = hidden_s_valid_dim - 1
            z = torch.zeros([1, self.ip_idx_dim - prob_valid_dim], device=DefaultValues.device)

            ip_idx1_probs = (
                    self.ip1_idx_output(all_pre_hidden, last_hidden).unsqueeze(0) +
                    self.ip1_idx_fc(last_hidden)[:,:prob_valid_dim]
            )

            ip_idx2_probs = (
                    self.ip2_idx_output(all_pre_hidden, last_hidden).unsqueeze(0) +
                    self.ip2_idx_fc(last_hidden)[:,:prob_valid_dim]
            )
            ...
            fn_para_mean = torch.round(
                torch.log(
                    self.fn_para_mean_output(last_hidden).relu()+1
                )*self.para_scale-0.5+1e-6
            )
            fn_para_std = torch.clamp(
                self.fn_para_std_output(last_hidden), min=1e-2)
...
class LSTM_crit(nn.Module):
    def __init__(
            ...
            lstm_hidden_size: int = 128,
            lstm_num_layers:int = 3,
            save_path:str = get_model_state_dict_path("LSTM_crit")
        ...
        self.register_buffer('hidden_states_buffer', hsb )"""
        self.hidden_states_buffer = None
        self.value_output = nn.Linear((self.total_amount_of_factors + DefaultValues.formula_profile_length)*self.lstm_hidden_size, 1)
        # 初始化权重
        self.initialize_weights()
    ...
    def initialize_weights(self):
        ...
        # Initialize output layers with smaller weights
        nn.init.uniform_(self.value_output.weight, -0.01, 0.01)
    ...
    def forward(self, input_state: torch.Tensor):
        ...
        for i in range(b):
            ...
            v = self.hidden_states_buffer.view(1,-1)
            values.append(self.value_output(v))
    ...
    def load(self, name: str):
        self.load_state_dict(torch.load(os.path.join(self.save_path, name + ".pth")))