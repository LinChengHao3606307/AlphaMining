...
class LSTM_sa_ag(nn.Module):
    def __init__(
            ...
            lstm_hidden_and_econ_dim: int = 128,
        ...
        #all EBD
        self.factors_embedding = nn.Embedding(total_amount_of_factors, lstm_hidden_and_econ_dim)
        self.fn_embedding = nn.Embedding(total_types_of_fn, lstm_hidden_and_econ_dim)
        self.tree_pos_embedding = nn.Embedding(2 ** formula_profile_length + 1, lstm_hidden_and_econ_dim)

        self.formula_profile_length = formula_profile_length
        self.profile_fuser = nn.Sequential(
            nn.Linear(lstm_hidden_and_econ_dim + 2, lstm_hidden_and_econ_dim * 2),
            nn.Tanh(),
            nn.Linear(lstm_hidden_and_econ_dim * 2, lstm_hidden_and_econ_dim)
        )

        encoder_layer = nn.TransformerEncoderLayer(
            d_model=lstm_hidden_and_econ_dim,
            nhead=enco_nhead,
            dim_feedforward=4 * lstm_hidden_and_econ_dim,
        ...
        self.bilstm_summerizer = nn.LSTM(
            input_size=lstm_hidden_and_econ_dim,
            hidden_size=lstm_hidden_and_econ_dim,
        ...
        self.fn_para_mean_output = nn.Linear(lstm_hidden_and_econ_dim * 2, DefaultValues.fn_profile_length - 3)
        self.fn_para_std_output = nn.Linear(lstm_hidden_and_econ_dim * 2, DefaultValues.fn_profile_length - 3)

        self.lstm_hidden_and_econ_dim = lstm_hidden_and_econ_dim
    ...
    def initialize_weights(self):
        ...
        for embedding in [self.factors_embedding, self.tree_pos_embedding, self.fn_embedding]:
    ...
    def _proceed(self, all_formulas, idx, t_shift, t_span, current_tree_pos): #num_layers, b, hidden_size
        self_tree_pos_idx = -1
        for i, v in enumerate(current_tree_pos):
            self_tree_pos_idx += v * 2**i
        self_tree_pos_emb = self.tree_pos_embedding(
            torch.LongTensor([self_tree_pos_idx+1]).to(DefaultValues.device)
        )
        if idx.detach().cpu().item() < 0:
            #print(f"EBD calling factors_embedding with input = {-idx.detach().cpu().item()-1}")
            self_content = self.factors_embedding(-idx-1)
        else:
            type_idx, id1, t_shift1, t_span1, id2, t_shift2, t_span2 = all_formulas[idx.item()].split(1)
            self_content = self.fn_embedding(type_idx)

        self_fn_type_emb = torch.cat([self_content, t_shift.unsqueeze(0), t_span.unsqueeze(0)], dim=-1)
        self_fn_type_emb:torch.Tensor = self.profile_fuser(self_fn_type_emb)

        self.emb_seq[idx].append(self_tree_pos_emb.unsqueeze(0) + self_fn_type_emb.unsqueeze(0))
        if idx.detach().cpu().item() < 0:
            return
        self._proceed(
            all_formulas, id1, t_shift1+t_shift, t_span1+t_span, current_tree_pos+[1]
        )
        if type_idx in DefaultValues.fn_of_2_ip:
            self._proceed(
                all_formulas, id2, t_shift2+t_shift, t_span2+t_span, current_tree_pos+[0]
            )
        return
    ...
    def forward(self, input_state: torch.Tensor):
        ...
        for i in range(b):
            ...
            valid_dim = self.total_amount_of_factors + state_l

            # list[ list[ [1,1,self.lstm_hidden_and_econ_dim] ] ]
            self.emb_seq = [[] for _ in range(valid_dim)]
            self._proceed(
            ...
            input_seq = []
            for t in self.emb_seq:
                input_seq += t
            input_seq = torch.cat(input_seq,dim=1)
            # Transformer编码 [batch_size=1, src_seq_len, d_model]
            encoded = self.transformer_encoder(input_seq)
            out, _ = self.bilstm_summerizer(encoded)

            out = out[:, -1]
            # action output
            fn_types_probs = F.softmax(self.fn_types_output(out), dim=-1)

            prob_valid_dim = valid_dim - 1
