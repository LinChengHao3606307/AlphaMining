...
def get_sep_idx(df:pd.DataFrame, dates:list[str]=["20211231","20221231"])->tuple[int,...]:
    ...
    return tuple(result)
"""
     [test_loss: 0.123385]______[vali_loss: 1.820685]
     [test_loss: 142.933772]______[vali_loss: 10.278810]
     [test_loss: 0.080496]______[vali_loss: 0.682441]
     [test_loss: 0.257093]______[vali_loss: 1.611713]
     [test_loss: 0.194770]______[vali_loss: 2.770443]
     [test_loss: 0.147347]______[vali_loss: 3.323560]
     [test_loss: 0.699829]______[vali_loss: 1.311955]
 cur [test_loss: 2.521047]______[vali_loss: 3.071075]
"""
def train(lr:float=1e-3,num_epoch:int=100,num_episode_per_epoch:int=5,num_steps_per_episode:int=500):
    # 上游布置

    # 上游策略网络
    from main.Agent.models.LSTM_ag import LSTM_ag
    from main.Agent.models.LSTM_p_ag import LSTM_p_ag
    from main.Agent.models.LSTM_sa_ag import LSTM_sa_ag
    from main.Agent.models.ENCO_ca_ag import ENCO_ca_ag
    from main.Agent.models.Value_est import Value_est
    upper_m = LSTM_ag().to(DefaultValues.device)
    upper_m_ip_state_type = "tpv"
    upper_crit = Value_est(main_model=LSTM_ag()).to(DefaultValues.device)

    # 上游主体
    from main.Agent import HybridPPO
    agent = HybridPPO(policy=upper_m, state_type=upper_m_ip_state_type, value_net=upper_crit, lr=lr,buffer_size=num_steps_per_episode)

    # 下游布置
    from main.Env import Env
    from main.Env.DataSet.data_sets.real_data import get_stock_data
    ...
    from main.Env.Criterion.customed_cri import CustomizedCri
