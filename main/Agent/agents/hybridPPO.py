...
# using PPO
class HybridPPO:
    def __init__(self, policy=None, value_net=None,
    ...
    def act(self, state:Formula):
        ...
        if self.policy is None:
            raise ValueError("Policy network not set. Call set_policy() first.")
    ...
    def update(self, batch_size=32, epochs=5):
        ...
        for _ in range(epochs):
            ...
            for start in range(0, states.size(0), batch_size):
                ...
                # 评估当前策略
                if self.policy is None or self.value_net is None:
                    raise ValueError("Policy or value network not set. Call set_policy() and set_value_net() first.")
                ...
                if mean is not None and batch_continuous_actions is not None:
                    dist = Normal(mean, std)
                ...
                total_log_prob = discrete_log_probs #+ continuous_log_prob
                total_entropy = torch.stack(discrete_entropies).sum(dim=0) #+ continuous_entropy
                ...
                # 梯度更新
                if self.optimizer_actor is not None:
                    self.optimizer_actor.zero_grad()
                    loss.backward()
                    #nn.utils.clip_grad_norm_(self.policy.parameters(), 0.5)
                    self.optimizer_actor.step()

                if self.optimizer_critic is not None:
    ...
    def _compute_returns(self, states, use_value_net = 0.6, use_mean = True, use_std = True, use_ratio_c = False):
        ...
        if self.value_net is None:
            raise ValueError("Value network not set. Call set_value_net() first.")
